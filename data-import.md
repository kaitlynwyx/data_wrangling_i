...
``` r
select(litters_df, GROUP = group, litterNum = litter_number)
```
...
``` r
rename(litters_df, GROUP = group, litterNum = litter_number)
```

    ## # A tibble: 49 × 8
    ##    GROUP litterNum       gd0_weight gd18_weight gd_of_birth pups_born_alive
